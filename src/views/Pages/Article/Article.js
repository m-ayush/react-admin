...
function UserRow(props) {
  ...
  const userLink = `/edit-article/${user.id}`;
  const viewLink = `/view-article/${user.id}`;
      ...
      <div className="animated fadeIn">
        <Row>
          <Col xl={12}>
            <Card>
              <CardHeader>
                <i className="fa fa-align-justify"></i> Article{" "}
                <small className="text-muted">list</small>
                <div className="card-header-actions">
                  <button
                    className="btn btn-primary m-btn m-btn m--align-right"
                    onClick={() => this.props.history.push("/add-article")}
                  >
                    <span>
